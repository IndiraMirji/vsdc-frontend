...
const Attendance = () => {
  ...
  return (
    <div className="attendance-container">
      ...
      <div className="attendance-summary">
        <div className="summary-card"><h3>Total Marked</h3><p className="summary-value">{attendance.length}</p></div>
        <div className="summary-card present"><h3>Present</h3><p className="summary-value">{attendance.filter(a => a.status === "Present").length}</p></div>
        <div className="summary-card absent"><h3>Absent</h3><p className="summary-value">{attendance.filter(a => a.status === "Absent").length}</p></div>
        <div className="summary-card leave"><h3>On Leave</h3><p className="summary-value">{attendance.filter(a => a.status === "Leave").length}</p></div>
      </div>

      <div className="form-card">
        <h2>{editingAttendance ? "Edit Attendance" : "Mark Attendance"}</h2>
        <div className="warning-box"><FaLock /> Records lock 1 hour after marking.</div>
        <form onSubmit={handleMarkAttendance}>
          <div className="form-row">
            <div className="form-group">
              <label>Staff Member</label>
            ...
            <div className="form-group">
              <label>Status</label>
              <select value={attendanceData.status} onChange={(e) => setAttendanceData({ ...attendanceData, status: e.target.value })}>
                <option value="Present">Present</option>
                <option value="Absent">Absent</option>
                <option value="Leave">Leave</option>
                <option value="Halfday">Half Day</option>
            ...
            {(attendanceData.status === "Present" || attendanceData.status === "Halfday") && (
              <>
                <div className="form-group"><label>Check In</label><input type="time" value={attendanceData.checkInTime} onChange={(e) => setAttendanceData({ ...attendanceData, checkInTime: e.target.value })} /></div>
                <div className="form-group"><label>Check Out</label><input type="time" value={attendanceData.checkOutTime} onChange={(e) => setAttendanceData({ ...attendanceData, checkOutTime: e.target.value })} /></div>
              </>
            )}
          </div>
          <div className="form-actions">
            <button type="submit" className="btn-primary">{editingAttendance ? "Update Record" : "Save Attendance"}</button>
            <button type="button" className="btn-cancel" onClick={resetForm}>Cancel</button>
          </div>
        </form>
      </div>
      ...
      <div className="attendance-table-container">
        <table className="attendance-table">
          <thead>
            <tr><th>Staff Name</th><th>Status</th><th>Check In</th><th>Check Out</th><th>Action</th></tr>
          </thead>
          <tbody>
            {attendance.length > 0 ? (
              attendance.map((rec) => {
            ...
            ) : (<tr><td colSpan="5" className="empty-state">No attendance records for this date.</td></tr>)}
          </tbody>
        </table>
      </div>
    </div>
